function [testData]=speakerIdentifyPer_wyf(target,testData,nameList,gmm, useIntGmm)
% speakerIdentify: speaker identification using GMM parameters
%	Usage: [recogRate]=speakerIdentifyPer_wyf(target,testData,nameList, gmm, useIntGmm)
...
for i=1:dayNum,
    ...
    for j=1:length(testData(i).sentence),
        if testData(i).sentence(j).predictedSpeaker==target
